...
app.get("/jokes/:id", (req, res) => {
    ...
    const specificJoke = getJoke(id);
    res.json(specificJoke);
});

//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) => {
    const filteredJokes = getJokes(req.query.type);
    res.json(filteredJokes);
});

//4. POST a new joke
app.post("/jokes", (req, res) => {
    const newJoke = createJoke(req.body.text, req.body.type);
    res.json(newJoke);
});
...
app.put("/jokes/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const updatedJoke = putJoke(id, req.body.text, req.body.type);
...
app.patch("/jokes/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const updatedJoke = patchJoke(id, req.body.text, req.body.type);
    res.json(updatedJoke);
});
...
app.delete("/jokes/:id", (req, res) => {
    ...
    if (deleteJoke(id)) {
        ...
        res.status(404).json(`Joke ${id} not found`);
    }
});
...
app.delete("/all", (req, res) => {
    ...
    if (req.query.masterKey === masterKey) {
        ...
        res.status(401).json(`Action not allowed`);
    }
});
...
function getRandomJoke() {
    const randomJokeIndex = getRandomNumber(jokes.length);
...
function patchJoke(id, text, type) {
    const searchIndex = jokes.findIndex((joke) => joke.id === id);
    if (searchIndex !== -1) {
        let existingJoke = jokes[searchIndex];
        if (text) {
            existingJoke.jokeText = text;
        }

        if (type) {
            existingJoke.jokeType = type;
        }
        jokes[searchIndex] = existingJoke;
        return existingJoke;
    } else {
        return { error: `Joke ${id} not found` };
    }
